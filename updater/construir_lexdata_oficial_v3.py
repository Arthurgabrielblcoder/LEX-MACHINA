...
r"""
LEX MACHINA
PIPELINE OFICIAL V3 - PREFLIGHT CONFIÁVEL + CORPUS CANÔNICO
...
Arquitetura da V3
=================
- A fonte principal é a manifestação "Current" / "Compilação Monovigente"
  informada no campo encoding do próprio JSON-LD.
...
Uso:
    python construir_lexdata_oficial_v3.py D:\

Somente depois de obter PREFLIGHT 9/9:
    python construir_lexdata_oficial_v3.py D:\ --full
"""
...
PASTA_PUBLICADA = (
    "99_LEXDATA_ESP32_OFICIAL_V3_TESTE"
)
...
def escrever_relatorio_preflight(
    ...
    linhas = [
        ...
        (
            "- a fonte principal precisa ser Current/Compilação Monovigente."
        ),
        "",
...
def executar_full(
    ...
    linhas_artigos = [
        "#LEXMACHINA|ARTIGOS|OFICIAL3",
        "#ID|ARTIGO|OFFSET|TAMANHO",
    ]

    linhas_menu = [
        "#LEXMACHINA|MENU|OFICIAL3",
    ...
    (
        ...
        (
            "LEX MACHINA\n"
            "VALIDAÇÃO TOTAL DE OFFSETS - OFICIAL3\n"
            + "=" * 84
            + "\n\n"
            + "\n".join(
                validacao_offsets
    ...
    relatorio = [
        "LEX MACHINA",
        "CORPUS OFICIAL V3 - RESULTADO COMPLETO",
        "=" * 92,
        "",
        f"GERADO EM: {agora()}",
        "",
...
def argumentos():
    parser = argparse.ArgumentParser(
        description=(
            "LEX MACHINA - pipeline oficial V3."
        )
    )
...
def main():
    ...
    if not args.full:
        ...
        print(
            (
                "Se o RELATORIO_PREFLIGHT_V3.txt confirmar 9/9, "
