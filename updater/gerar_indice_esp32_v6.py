...
# ============================================================
# LEX MACHINA
# GERADOR DE ÍNDICES ESP32 V6 - MODO TESTE SEGURO
#
# OBJETIVO
# -------
...
#   MENU.IDX
#   META.JSON
#   RELATORIO_INDICE_ESP32_V6.txt
#   AMOSTRAS_OFFSETS_V6.txt
#
...
# ============================================================
# PARSER DE CABEÇALHOS DE ARTIGO V6
#
# Correções específicas sobre a V5:
#
# 1) aceita o formato legado:
#       Art . 1º
#    usado em páginas antigas do Planalto;
#
# 2) mantém suporte a:
#       Art.
#       1º ...
#
# 3) penaliza fortemente referências em minúsculas:
#       art. 233 da Lei ...
#       artigo 36, quantos bastem ...
#
# 4) descarta candidatos de baixa confiança ANTES da
#    deduplicação;
#
# 5) em empate, prefere a ocorrência MAIS ANTIGA, não a mais
#    nova. Isso impede referências posteriores de substituírem
#    o cabeçalho real;
#
# 6) o validador de offset não decodifica mais um bloco fixo
#    de 2048 bytes. Esse bloco podia terminar no meio de um
#    caractere UTF-8 e gerar falso "lido=None".
#
# 7) o tamanho do artigo termina no próximo CABEÇALHO
#    SELECIONADO, não em qualquer referência rejeitada.
# ============================================================
...
PADRAO_ARTIGO_DIRETO = re.compile(
    ...
    r"\s*"
    r"(?P<ord>º|°|o)?"
    # Sufixo apenas se estiver colado ao número/ordinal.
    r"(?P<suf>-[A-Za-z]{1,2})?"
    r"(?=$|[\s\.,;:\)\]\-–—])",
    re.IGNORECASE,
)
...
MIN_PONTOS_CANDIDATO = 8.0


# Guardas conservadoras contra regressões grosseiras.
REGRESSION_GUARDS = {
    "CF88": 270,
    "CC2002": 2050,
    "CPC2015": 1000,
...
def pontuar_contexto_artigo(
    ...
    """
    Cabeçalhos oficiais normalmente usam "Art." com A maiúsculo.

    Referências internas que foram quebradas pelo HTML costumam
    aparecer como:
        art. 233 da Lei...
        artigo 36, quantos bastem...

    A V5 aceitava esses casos como artigos reais.
    """
...
def criar_candidato_direto(
    ...
    capitalizado = (
        token_art_capitalizado(
            match
        )
    )

    pontos = pontuar_contexto_artigo(
        resto,
        "DIRETO",
        capitalizado,
    )
...
# SELEÇÃO V6
#
# O texto local já passou pelo updater e, nas fontes Planalto,
# as redações explicitamente riscadas foram removidas.
#
# Portanto, quando sobram duas ocorrências de uma mesma chave:
#   1) maior pontuação local;
#   2) em empate, ocorrência mais ANTIGA.
#
# Essa regra corrige o falso Art. 1 da Lei 8.212/1991 que,
# na V5, era uma referência posterior em minúsculas.
# ============================================================
...
#
# Lê LINHAS COMPLETAS. A V5 lia exatamente 2048 bytes;
# quando o byte 2048 cortava um caractere UTF-8 no meio,
# o decoder caía para cp1252 e "Art. 1º" virava algo como
# "Art. 1Âº", causando falso lido=None.
# ============================================================

def linhas_texto_a_partir_offset(
    dados,
    encoding,
    offset,
    max_linhas=8,
...
def cabecalho_no_offset(
    ...
    linhas = linhas_texto_a_partir_offset(
        dados,
        encoding,
        offset,
        max_linhas=8,
...
def gerar_indices(
    ...
    (
        ...
        (
            "LEX MACHINA\n"
            "AMOSTRAS REAIS LIDAS PELOS OFFSETS V6\n"
            + "=" * 78
            + "\n\n"
            + "\n".join(
                amostras
    ...
    relatorio = [
        "LEX MACHINA",
        "GERAÇÃO DE ÍNDICES PARA ESP32 - VERSÃO 6",
        "MODO TESTE SEGURO",
        "=" * 78,
        "",
        f"GERADO EM: {agora()}",
    ...
    for detalhe in detalhes:
        relatorio.append(
            (
                ...
                f"| baixa_conf={detalhe['baixa_confianca']} "
                f"| duplicados={detalhe['duplicados']} "
...
def main():
    ...
    if (
        ...
        print(
            "✓ ÍNDICE V6 APROVADO AUTOMATICAMENTE"
        )
