...
RELATORIO_TXT = (
    PASTA_RELATORIOS
    / "RELATORIO_DOWNLOAD_PRIORIDADE_B.txt"
)

RELATORIO_JSON = (
    PASTA_RELATORIOS
    / "RELATORIO_DOWNLOAD_PRIORIDADE_B.json"
)
...
def argumentos():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "as normas ausentes de Prioridade B."
        )
    )
...
def detectar_existencia(
    ...
    arquivo_sugerido = normalizar_nome(
        item.get(
            "arquivo_sugerido",
            ""
        )
...
def gerar_relatorio(
    ...
    linhas = [
        "LEX MACHINA",
        "DOWNLOAD AUTOMÁTICO - PRIORIDADE B",
        "=" * 78,
        "",
        f"GERADO EM: {agora()}",
        f"DESTINO: {destino}",
...
def main():
    ...
    print(
        "DOWNLOAD PRIORIDADE B FINALIZADO"
    )
